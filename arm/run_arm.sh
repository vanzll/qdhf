...
for method in "${!noisy_methods[@]}"; do
    ...
    for param in "${params[@]}"; do
        ...
        qd_scores[$method,$param]=()
        coverage_scores[$method,$param]=()
        ...
        for seed in "${seeds[@]}"; do
            ...
            qd_scores[$method,$param]+="$qd_score "
            coverage_scores[$method,$param]+="$coverage "
        ...
        qd_values=(${qd_scores[$method,$param]})
        coverage_values=(${coverage_scores[$method,$param]})

        qd_mean=$(echo "${qd_values[@]}" | awk '{for(i=1;i<=NF;i++)s+=$i} END {print s/NF}')
        qd_std=$(echo "${qd_values[@]}" | awk '{for(i=1;i<=NF;i++)s+=$i; sumsq+=$i*$i} END {print sqrt(sumsq/NF - (s/NF)^2)}')

        coverage_mean=$(echo "${coverage_values[@]}" | awk '{for(i=1;i<=NF;i++)s+=$i} END {print s/NF}')
        coverage_std=$(echo "${coverage_values[@]}" | awk '{for(i=1;i<=NF;i++)s+=$i; sumsq+=$i*$i} END {print sqrt(sumsq/NF - (s/NF)^2)}')
